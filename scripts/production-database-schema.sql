...
-- This script creates all necessary tables, relationships, and admin setup
...
DROP TABLE IF EXISTS hire_bookings CASCADE;
DROP TABLE IF EXISTS real_estate_bookings CASCADE;
...
CREATE TABLE real_estate_bookings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_real_estate_properties_type ON real_estate_properties(type);
CREATE INDEX idx_real_estate_properties_city ON real_estate_properties(city);
CREATE INDEX idx_real_estate_properties_is_available ON real_estate_properties(is_available);

CREATE INDEX idx_real_estate_bookings_property_id ON real_estate_bookings(property_id);
CREATE INDEX idx_real_estate_bookings_customer_id ON real_estate_bookings(customer_id);
CREATE INDEX idx_real_estate_bookings_status ON real_estate_bookings(status);

CREATE INDEX idx_hire_bookings_customer_id ON hire_bookings(customer_id);
CREATE INDEX idx_hire_bookings_service_type ON hire_bookings(service_type);
...
CREATE POLICY "wishlist_items_customer_access" ON wishlist_items FOR ALL USING (customer_id = auth.uid() OR is_admin());

-- Review policies
CREATE POLICY "reviews_public_read" ON product_reviews FOR SELECT USING (is_approved = true);
CREATE POLICY "reviews_customer_manage" ON product_reviews FOR ALL USING (customer_id = auth.uid() OR is_admin());
